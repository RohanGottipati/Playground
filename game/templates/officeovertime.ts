...
export function buildOfficeovertimeSpec(
  ...
  return {
    ...
    rules: {
      ...
      howToPlay: [
        `${titleCase(ammo)}s come fast — the covers are narrow to climb on and the volleys barely pause.`,
        "A spill splits the long middle stretch; you must clear it between volleys.",
        "Low shots skim the floor — jump them; high shots pass by at head height — stay grounded.",
        "Touching the machine hurts. The flag is planted just in front of it.",
      ],
      controls: BASE_CONTROLS,
      tip: "Count the rhythm: volley, jump, sprint. Mistimed jumps end runs here.",
    },
    world: baseWorld(),
    player: basePlayer(),
    entities,
