 ...
 * photo is a seeded coin flip, not an AI decision.
 */
...
/** Ascending-difficulty campaign order — also the arcade seed order. */
...
/**
 * Seeded template pick so a stored seed always replays the same game.
 *
...
export function templateForSeed(
  ...
  if (weights.size === 0) {
    for (const id of TEMPLATE_IDS) weights.set(id, baseWeight(id));
  }
...
export function buildTemplateSpec(
  ...
  const template =
    options.forceTemplate ?? templateForSeed(seed, options.recentTemplates);
